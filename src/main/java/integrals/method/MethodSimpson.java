...
/**
 * Это расчет интеграла методом Симпсона.
 ...
 *
 * Порядок точности данного метода = 4
 *
 ...
 */
public class MethodSimpson implements DescribingMethod {
    /**
     * поля класса
     */
    private static final int ORDER_OF_ACCURACY = 4;
     ...
     */
    public MethodSimpson(Integrable function, double from, double to) {
     ...
     *
     * Количесво шагов должно быть четным числом => происходит проверка на данное условие и вывод сообщения
     *
    ...
    public double solve( double step) {
        return solve(this.function, this.from, this.to, step);
    }
    ...
    public double accurateSolve(double step, double degreeOfAccurate) {
        return accurateSolve(this.function, this.from, this.to, step, degreeOfAccurate);
    }
     ...
     * @param to - до этого значения идет подсчет
     * @param step - шаг(размер) итерирования по оси ОХ
     * @return - возвращается результат интегрированния или -1 если количество шагов - нечетное число
     */
    public static double solve(Integrable function, double from, double to, double step){
        if (!((to - from)/step % 2 == 0)) {
            System.out.print("is not even count   ");
            return -1;
        }
        double point = from;
        ...
        for (int i = 1; i < (to - from)/step; i++) {
            point += step;
            if (i % 2 == 0) sum += 2 * function.calculate(point);
            else sum += 4 * function.calculate(point);
        }
        point += step;
